...
//Suppression du using namespace std puisque son usage est deconseille.

int gagne(int c,int l,int J,int tab[7][6])  // renvoie J si le joueur gagne, sinon 1.
{
	//Optimisation du nombre de variables utilisees.
	int j[4];
	int k[3];
	...
	//Modification de la valeur renvoye si aucun joueur ne gagne.
	...
	if (gagne(c,nbJetonsByColonne[c],J,tab)==-1)    // si ce que je joue ne me fait pas tout de suite gagner
	{
		if (I!=0)
		{
			...
			if (J==3)
				J=1;
			if (r==7)
				s=J;   // si toutes les solutions menent a perdre...
			nbJetonsByColonne[c]--;
			tab[c][l]=0;
		}
	}
...
void jeu(int mode)
{
	...
	int tab[7][6];
	int iaOrdinateur,J;
	int g=-1,c,l,compteur=0;                //g=0 => le jeu continue ; g=1 => ordi gagne ; g=2 => tu gagnes ; g=3 => egalite


	//Ajout pour gerer si on est en mode 2 joueurs ou en mode contre l'ordinateur
	bool mode2joueurs=false;
	for (int i=0; i<7; i++)  // mise a zero du tableau et des colonnes
	{
	...
	else
	{
		if(mode==2)
		{
			...
			std::cin>>tabJoueurs[0];
			iaOrdinateur = 1 ;                 //intelligence ordi
		}
	}

	//Determine qui debute la partie au hasard au lieu de choisir.
	srand(time(NULL));
	J=rand()%2;
	if(J==0)
	...
	while (g==-1)         //  tant que personne n'a gagner...
	{
        if (compteur==42)  // egalite
		{
			g=-1;
			break;
		}
		...
		{
			std::cout<<std::endl<<"Laisse-moi le temps de reflechir..."<<std::endl;
			int c3[7]={0,6,5,1,2,4,3},r[7];   // c3 designe l'ordre de preference, du moins vers le plus.
	...
	//Modification des messages affich�s et des valeurs pour chacune des cases puisque que la num�rotation des joueurs a �t� chang�e.
	switch(g)
	{
...
int main(int argc, char *argv[])
{
    ...
    {*/
		...
		//Redimensionnement de la fenetre console pour un meilleur affichage.
		...
		//Ajout d'un menu et de sa gestion pour les differents modes et pour quitter automatiquement.
		std::cout << std::endl << "                               *****************" << std::endl << "                               *  puissance 4  *" << std::endl << "                               *****************" << std::endl << std::endl;
		std::cout<<"--------------------------------------------------------------------------------"<<std::endl;
		std::cout<<"                                 Introduction"<<std::endl;
		std::cout<<"--------------------------------------------------------------------------------"<<std::endl;
		std::cout<<"Apres les apres de plusieurs guerres mondiales entre les deux grandes nations de la planete Pion, situation s'etait amelioree et le dialogue retrouva sa place parmi les pays\n"<<std::endl;
		std::cout<<"Pousse par la population, les grands gouvernements etablirent une commission afin de trouver LA solution permettant de conserver la paix.\n"<<std::endl;
		std::cout<<"Apres des siecles de reflexions, les sages mirent en place un concept qui permet d'allier strategies et ruses pour assoir l'autorite de la nation au coeur pure.\n"<<std::endl;
		std::cout<<"Les grands vaisseaux de guerres furent demis de leurs armes.\n"<<std::endl;
        std::cout<<"De grands champs stellaires furent amenages et une nouvelle aire vit le jour Les sages appellent ce système PUISSANCE 4.\n"<<std::endl;
		std::cout<<"Les grands vaisseaux de guerres furent demis de leurs armes."<<std::endl;
        std::cout<<"--------------------------------------------------------------------------------"<<std::endl;
		...
		//Appel la methode pour demarrer le jeu si l'utilisateur n'a pas choisi l'option quitter dans le menu.
		if(c!=0)
		{
			jeu(c);
		}
